...
class DocumentBriefSerializer(serializers.ModelSerializer):
    ...
    def get_file_url(self, obj):
        ...
        if user == obj.uploaded_by or user == case.user or user.is_staff:
            ...
            # Format for Vercel serverless function: /api/proxy-documents?type=case&disputeId=X&docId=Y&access=token
            from django.conf import settings
            frontend_url = getattr(settings, 'FRONTEND_URL', 'http://localhost:5173')
            return f"{frontend_url}/api/proxy-documents?type=case&disputeId={case.id}&docId={obj.id}&access={temp_token}"
